...
⚠️ **tool result 첨부파일:** tool 메시지의 첨부파일은 LangChain / AutoGen 계열 패턴을 따라 `content` 배열 안에 `input_image`/`input_audio` 블록으로 함께 전송된다. 즉, 텍스트 tool 결과와 멀티모달 첨부파일이 한 메시지에서 같이 LLM에 전달된다.
...
⚠️ **function_call_output 제약:** Responses API의 `function_call_output` 아이템은 `output`이 문자열(string)만 가능하다. 그래서 tool result 첨부파일은 `function_call_output` 뒤에 별도의 user 메시지로 전송한다.

전송 패턴은 다음과 같다:

1. `{"type": "function_call_output", "call_id": ..., "output": "..."}`
2. `{"role": "user", "content": [{"type": "input_text", "text": "[Tool result attachments from {tool_name}]"}, {"type": "input_image", ...}, ...]}`

이 패턴을 쓰면 LLM이 tool 실행 문맥을 유지한 채 이미지/오디오를 함께 분석할 수 있다. LangChain, AutoGen, OpenAI computer use guide와 같은 ecosystem pattern이다.
...
5. **OpenAI tool result attachments**: Chat Completions의 tool role `content` multipart 패턴과 Responses API의 `function_call_output` + user message 패턴은 모두 OpenAI의 공식 “tool attachments” 전용 스펙이 아니라 ecosystem convention이다. 특히 Responses API는 `function_call_output.output`이 string-only라서 attachments를 분리해야 한다.
